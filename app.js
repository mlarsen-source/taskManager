...
app.post("/createTask", async (req, res) => {
  ...
  await conn.query(`INSERT INTO tasks (title, dueDate, location, description, priority, type, view) 
    VALUES (?, ?, ?, ?, ?, ?, 1)`, [task.title, task.dueDate, task.location, task.description, task.priority, task.type]);
...
app.post("/updateTask/:taskId", async (req, res) => {
  ...
  await conn.query(
    ...
    [req.body.title, req.body.dueDate, req.body.location, req.body.description, req.body.priority, req.body.type, req.body.status, viewValue, taskId]
  );
